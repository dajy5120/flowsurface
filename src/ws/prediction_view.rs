...
fn binance_block<'a>() -> Element<'a, PredictionMsg> {
    use super::pm_binance_readout as pm;

    let st = pm::snapshot();
    let mut c = column![sec("Binance Prediction Trading · BTC 5 分钟涨跌（实时）")].spacing(6);

    if !st.present {
        c = c.push(text(
            "暂无快照。启动录制器：python -m factory.prediction.pm_recorder
             （需 .env 里的 BINANCE_PREDICTION_* 凭证 + Prediction Trading 权限）",
        ).size(11).color(C_DIM));
        return c.into();
    }

    // ── 录制状态：**这份数据没有第三方历史源，断了就是永久缺口** ──
    // 所以把它放在最显眼处，而不是藏在角落。
    let alive = pm::recording_alive(&st);
    let (dot, dc, txt) = if alive {
        ("●", C_GREEN, format!(
            "录制中  {} 条簿 / {} 轮  WS {} 条（重订 {} 次）",
        ...
        ("✗", C_RED, format!(
            ...
            if st.rec.last_error.is_empty() { String::new() }
            else { format!("  错误 {}", st.rec.last_error) }))
    };
    c = c.push(row![
        text(format!("{dot} ")).size(14).color(dc),
        text(txt).size(12).color(if alive { C_TXT } else { C_RED }),
    ]);
    if !alive {
        c = c.push(
            text("⚠ 这份数据没有第三方历史源（不像永续有 Tardis）——停多久就永久缺多久")
                .size(11).color(C_GOLD),
        );
    }

    // ── 当前这一轮 ──
    if let Some(r) = &st.round {
        let left = if r.secs_left >= 0 {
            format!("{}:{:02}", r.secs_left / 60, r.secs_left % 60)
        } else {
            "—".into()
        };
        c = c.push(text(r.question.clone()).size(13).color(C_HEAD));
        c = c.push(row![
            cell(format!("剩余 {left}"), 110.0, if r.secs_left < 30 { C_GOLD } else { C_TXT }),
            cell(format!("起始价 {:.2}", r.start_price), 150.0, C_TXT),
            cell(format!("参与 {}", r.participants), 90.0, C_DIM),
            cell(format!("成交 {}", usd(r.volume)), 110.0, C_DIM),
            cell(format!("流动性 {}", usd(r.liquidity)), 120.0, C_DIM),
            cell(format!("费 {:.1}%", r.fee_bps / 100.0), 80.0, C_GOLD),
        ].spacing(4));
    }

    // ── 两本簿 ──
    if let Some(b) = &st.book {
        let px = |v: Option<f64>| v.map(|x| format!("{x:.2}")).unwrap_or_else(|| "—".into());
        c = c.push(row![
            cell("".into(), 60.0, C_DIM),
            cell("买价".into(), 70.0, C_DIM),
            cell("卖价".into(), 70.0, C_DIM),
            cell("买盘金额".into(), 110.0, C_DIM),
            cell("档数".into(), 90.0, C_DIM),
        ].spacing(4));
        for (name, bid, ask, notional, lv) in [
            ("涨 Up", b.up_bid, b.up_ask, b.up_notional, b.up_levels),
            ("跌 Down", b.down_bid, b.down_ask, b.down_notional, b.down_levels),
        ] {
            c = c.push(row![
                cell(name.into(), 60.0, C_TXT),
                cell(px(bid), 70.0, C_TXT),
                cell(px(ask), 70.0, C_TXT),
                cell(usd(notional), 110.0, C_TXT),
                cell(format!("{}/{}", lv.0, lv.1), 90.0, C_DIM),
            ].spacing(4));
        }
        if let Some(imb) = b.imbalance {
            let who = if imb > 0.0 { "押涨占优" } else { "押跌占优" };
            c = c.push(
                row![
                    text(format!("失衡 {imb:+.2}（{who}）")).size(13).color(sign_c(imb)),
                    text(format!(
                        "    按份数 {:+.2}",
                        b.imbalance_shares.unwrap_or(0.0)
                    )).size(11).color(C_DIM),
                ],
            );
            // **两个口径都显示**是有意的：实测按份数会指反——临近结算时便宜一侧
            // 用很少的钱堆出大量份额，那是彩票式挂单不是共识。看得见差多少，
            // 才不会在别处又按份数去算。
            c = c.push(
                text("失衡按**金额**算（Σ价×量）；按份数的仅供对照——临近结算时它会指反")
                    .size(10).color(C_DIM),
            );
        }
    }
    c = c.push(text(format!("快照 {}  刷新 {}", st.stamp, st.refreshed)).size(10).color(C_DIM));
    c.into()
}
...
pub fn pane_body<'a>() -> Element<'a, PredictionMsg> {
    ...
    body = body.push(binance_block());
    body = body.push(text("").size(6));
