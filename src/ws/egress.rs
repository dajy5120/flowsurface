...
/// 扫一轮。**只在后台线程里调**：起 systemctl 子进程，不可进渲染线程。
fn collect() -> Vec<Row> {
    ALL.iter()
        .map(|s| match s.kind {
            Kind::Foreign => {
                let pid = pid_of(s.unit);
                Row {
                    key: s.key.into(),
                    on: pid != 0,
                    enabled: false,
                    conns: (pid != 0).then(|| established(pid)),
                    ..Default::default()
                }
            }
            Kind::InProcess => Row {
                ...
                enabled: false,
                conns: Some(established(std::process::id())),
                ..Default::default()
            },
            Kind::Service => {
                let st = svcctl::query(s.unit);
                let pid = if st.active { main_pid(s.unit) } else { 0 };
                Row {
                    key: s.key.into(),
                    on: st.active,
                    enabled: is_enabled(s.unit),
                    conns: st.active.then(|| established(pid)),
