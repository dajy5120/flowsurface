...
mod tests {
    ...
    /// 构建一次满载尾窗的「最慢一帧」耗时（毫秒）。`n` = 尾窗行数。
    fn worst_frame_ms(n: i64) -> f64 {
    ...
    /// 尾窗构建开销随行数的增长（**基准，不是测试**，默认不跑）。
    ///
    /// ```sh
    /// cargo test tail_build_cost -- --ignored --nocapture
    /// ```
    ///
    /// ## 为什么把它从闸门里拿出来（T-01）
    ///
    /// 它原本断言 `worst < 8.0` 毫秒。**2026-09-11 整改 Q-02 时它红过一次**——
    /// 当时机器上同时在跑 clippy 与 release 构建，随后连跑三次全绿。
    ///
    /// 关键数据：200 行的典型耗时约 **0.6ms**，而阈值是 **8ms**——**余量约 13 倍，
    /// 照样被打穿**。这说明在任意负载下，挂钟绝对值断言**没有任何安全余量可言**：
    /// 一次调度停顿就能让单帧慢一个数量级。CI runner 是共享机器，比本机更不可控。
    ///
    /// 中途试过两种「更聪明」的判据，都不成立，记下来免得有人再走一遍：
    /// - **大/小耗时比值**：耗时里有一段与行数无关的固定开销 F，会把比值压扁。
    ///   实测 F≈0.13ms 时平方退化只到 ~27×（贴着阈值）；F 更大的机器上只到 ~5×，**漏掉**。
    /// - **边际成本比**（把 F 差掉，数学上对）：实测**线性情形下就在 0.8–1.9× 之间抖**，
    ///   而平方退化的信号约 2.5×——**信号与噪声重叠**，在亚毫秒尺度上分不开。
    ///   放大到 400/2000/4000 行也只是把重叠减轻，没有消除。
    ///
    /// **一条会偶发红的闸门，后果和永远红的闸门一样：人开始无视它。**
    /// 所以这里不再断言时间，只打印测量值——**它是给人看的基准，需要时手动跑**。
    /// 真要自动守复杂度，得换成「数操作次数」而不是「量墙上时间」，那是另一件事。
    #[test]
    #[ignore = "基准而非测试：挂钟断言在任意负载下都会偶发红（T-01）"]
    fn tail_build_cost_benchmark() {
        let (t400, t2000, t4000) = (worst_frame_ms(400), worst_frame_ms(2000), worst_frame_ms(4000));
        let c_low = (t2000 - t400) / 1600.0;
        let c_high = (t4000 - t2000) / 2000.0;
        eprintln!(
            "[P1] 400/2000/4000 行 = {t400:.3}/{t2000:.3}/{t4000:.3}ms；\
             边际 {c_low:.5} → {c_high:.5} ms/行（整帧预算 16ms）\n\
             线性时两段边际相当；若后段明显更贵，就是出现了超线性项，该查了。"
        );
    }
